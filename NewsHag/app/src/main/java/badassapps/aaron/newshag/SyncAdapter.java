...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        String data = "";
        try {
            URL url = new URL("https://us.api.battle.net/wow/character/sargeras/blimpie?fields=appearance&locale=en_US&apikey=wheces9zargz65mhza5jfv9nentuy2gg");
        ...
        for (int i = 0; i < result.getResults().size(); i++) {
            ...
            values.put("name", result.getResults().get(i).getTITLE());
            values.put("level", result.getResults().get(i).getURL());
            values.put("thumbnail", result.getResults().get(i).getIMAGE());
            values.put("realm", result.getResults().get(i).getABSTRACT());
